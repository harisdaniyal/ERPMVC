...
$(document).ready(function () {
    ...
    function save(row, IsCompleted) {
        ...
        dataObject.push(JSON.stringify({
            ...
            'OrderType': $("#txt_OrderType option:selected").text(),
            'OrderNo': $("#txt_importorderNo option:selected").val(),
            'ContainerNo': $("#txt_importContainerNo option:selected").val(),
            'HeadType': row.find("#txt_HeadType option:selected").text(),
            //'HeadName': row.find("#txt_HeadName option:selected").val(),
            ...
            if (response.success) {
                if (IsCompleted) {
                    ...
                    if (row.find(".txt_ID").val() == 0) {
                        $('#example tbody').prepend(` <tr class="bg-light tbl-valign-top"><td>
