$(document).ready(function myfunction() {
    ...
    $(document).on("click", ".btndlt", function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/Setup/ExportDeleteTrainID?Id=' + Id,
            async: false,
    ...
    function save(row, IsDeleted) {
        ...
        else if (row.find(".txt_TrainId").val() == '') {
            toastr.error('Please Enter TrainID !!')
            return false;
        }
        ...
        showLoader();
        fetch('/Setup/ExportTrainIdAsync', {
            method: 'POST',
            body: dataObject,
            headers: {
            ...
            if (response.success) {
                    ...
                    if (row.find(".txt_ID").val() == 0) {
                        $('#example').prepend(`<tr class="bg-light tbl-valign-top">
                              <td>
                                ...
                                <input type="text" style="width: 250px;" value="" class="form-control txt_TrainId" />
                            </td>
                ...
                hideLoader();
                toastr.success("TrainID Has Been Saved Successfully..")
