...
namespace BA_ERPMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        AccountServices _accountService = null;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    //var userObject = await UserManager.FindByIdAsync(model.Id);
                    //userObject.UserName = user.Email;
                    //userObject.Email = user.Email;
                    //userObject.CNIC = user.CNIC;
                    //userObject.MobileNo = user.MobileNo;
                    //userObject.Name = user.Name;
                    //userObject.AgencyType = user.AgencyType;
                    //userObject.Status = user.Status;

                    //if (model.Status == "APPROVED")
                    //{
                    //    userObject.ApprovedBy = UserDetail1.Email;
                    ...
                    //    else
                    //    {
                    //        userObject.AddedBy = UserDetail1.Email;
                    //        userObject.IsActive = false;
                    //    }

                    //}
                    //result = await UserManager.UpdateAsync(userObject);
                    //if (result.Succeeded)
                    //{
                    ...
                    //    UserManager.RemoveFromRole(model.Id, oldrole.Name);
                    //    await UserManager.AddToRoleAsync(model.Id, model.Role);
                    //    ViewBag.Message = "Successfull";
                    //    ViewBag.AgencyId = new SelectList(registerDropDownModel.AgencyList.Where(x => x.isActive == true).Select(x => new { x.AgencyID, x.AgencyName }), "AgencyID", "AgencyName");
                    //    return View(model);
                ...
                {
                    var isExists = _applicationDbContext.Users.Where(x => x.Email == model.Email).FirstOrDefault();
                    if (isExists != null)
                    {
                        ModelState.AddModelError(string.Empty, "User with same Email already exists");
                    }
                    else
                    {
                        Mapper.Reset();
                        Mapper.Initialize(config =>
                        {
                            ...
                            config.CreateMap<ApplicationUser, UserRegistrationViewModel>();
                        });

                        var userRegisterViewModel = Mapper.Map<ApplicationUser, UserRegistrationViewModel>(user);

                        string message = _userService.IsUserExists(userRegisterViewModel);
